function example_usage_multinmf_conv_mu()
...
Q_init = permute(abs(A_init).^2, [3 1 2]);

[Q_MU, W_MU, H_MU, cost] = multinmf_conv_mu(abs(X).^2, 500, Q_init, W_init, H_init, source_NMF_ind);


% Reconstruction of the spatial source images
fprintf('Reconstruction of the spatial source images\n');
Ie_MU = multinmf_recons_im(X, Q_MU, W_MU, H_MU, source_NMF_ind);
