function [W,H,A,Sigma_b,S,log_like_arr] = ...
    multinmf_inst_em(X, W0, H0, A0, Sigma_b0, source_NMF_ind, iter_num, SimAnneal_flag, Sigma_b_Upd_flag)

%
% [W,H,A,Sigma_b,S,log_like_arr] = ...
%    multinmf_inst_em(X, W0, H0, A0, Sigma_b0, source_NMF_ind, iter_num, SimAnneal_flag, Sigma_b_Upd_flag);
%
% EM algorithm for multichannel NMF decomposition in linear instantaneous mixture (stereo)
%
%
% input 
% -----
...
% A0                : initial (real-valued) mixing matrix [2 x J]
...
% A0                : estimated (real-valued) mixing matrix [2 x J]
...
for iter = 1:iter_num
    ...
    for j = 1:J
        Sigma_x(:,:,1,1) = Sigma_x(:,:,1,1) + abs(A(1,j)).^2 * sigma_ss(:,:,j);
        Sigma_x(:,:,1,2) = Sigma_x(:,:,1,2) + A(1,j) * conj(A(2,j)) * sigma_ss(:,:,j);
        Sigma_x(:,:,2,1) = conj(Sigma_x(:,:,1,2));
        Sigma_x(:,:,2,2) = Sigma_x(:,:,2,2) + abs(A(2,j)).^2 * sigma_ss(:,:,j);
    end;
    ...
    for j = 1:J
        ...
        Gs(:,:,j,1) = (conj(A(1,j)) * Inv_Sigma_x(:,:,1,1) + ...
                       conj(A(2,j)) * Inv_Sigma_x(:,:,2,1)) .* sigma_ss(:,:,j);
        Gs(:,:,j,2) = (conj(A(1,j)) * Inv_Sigma_x(:,:,1,2) + ...
                       conj(A(2,j)) * Inv_Sigma_x(:,:,2,2)) .* sigma_ss(:,:,j);
    ...
    for j1 = 1:J
        ...
        for j2 = 1:J
            bar_Rss(:,j1,j2) = mean(Gs_x(:,:,j1) .* conj(Gs_x(:,:,j2)) ...
                - (Gs(:,:,j1,1) * A(1,j2) + Gs(:,:,j1,2) * A(2,j2)) .* sigma_ss(:,:,j2), 2);
        end;
    ...
    for k = 1:K
        ...
        Gc_k_1 = (conj(bar_A(1,k)) * Inv_Sigma_x(:,:,1,1) + ...
                  conj(bar_A(2,k)) * Inv_Sigma_x(:,:,2,1)) .* sigma_cc_k;
        Gc_k_2 = (conj(bar_A(1,k)) * Inv_Sigma_x(:,:,1,2) + ...
                  conj(bar_A(2,k)) * Inv_Sigma_x(:,:,2,2)) .* sigma_cc_k;
        ...
        Vc(:,:,k) = abs(Gc_x_k).^2 + sigma_cc_k ...
            - real(Gc_k_1 * bar_A(1,k) + Gc_k_2 * bar_A(2,k)) .* sigma_cc_k;
    end;
    ...
    % re-estimate A
    A = squeeze(mean(real(bar_Rxs(:,:,:)), 1)) * inv(squeeze(mean(real(bar_Rss(:,:,:)), 1)));
        
    % re-estimate noise variances (if necessary)
    if Sigma_b_Upd_flag
        for f = 1:F
            Sigma_b(f) = 0.5 * real(trace(squeeze(bar_Rxx(f,:,:)) - A * squeeze(bar_Rxs(f,:,:))' - ...
                squeeze(bar_Rxs(f,:,:)) * A' + A * squeeze(bar_Rss(f,:,:)) * A'));
        end;
    end;
    ...
    for j = 1:J
        ...
        A(:, j) = A(:, j) / sqrt(A_scale);                           % Normalisation of mixing filters A
        W(:,source_NMF_ind{j}) = W(:,source_NMF_ind{j}) * A_scale;   % Energy transfer to W components
    end;
